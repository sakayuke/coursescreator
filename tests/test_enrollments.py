...
# 1. Admin and views students page for a course
...
# 2. Teacher who owns the course can view students page
def test_teacher_can_view_own_course_students_page(client, test_data):
...
# 3. Student blocked from viewing course students page
def test_student_cannot_view_course_students_page(client, test_data):
    login_user(client, test_data["student_id"])
    res_student = client.get(f"/courses/{test_data['course_id']}/students")
    assert res_student.status_code == 403

# 4. Admin successfully enrolls a student
...
# 5. Enrolled student sees course in /courses list
...
# 6. Unenrolled student does not see course in /courses list
...
# 7. Admin removes an enrolled student
...
# 8. Edge case: enroll non-existent student (400 Bad Request)
...
# 9. Edge case: enroll a teacher instead of student (400 Bad Request)
...
# 10. Edge case: non-existent course returns 404
